...
class SudokuPlotter():
    ...
    def empty_sudoku(self, ax: plt.axes) -> None:
        ...
        param: ax: plt.axes : Axes to draw sudoku on
        """
        ...
        # Coloring outer edge - green or red for solve or fail
        ...
        # If a proper sudoku draw thick lines onto grid
        # # If a proper sudoku draw thick lines onto grid
        # subgrid_len = np.sqrt(self.dimension)
        # step = int(subgrid_len)
        # if (self.dimension == 2) or (subgrid_len == step):
        #     step = int(step)
        ...
        ax.set_xticks(
            np.arange(1, self.dimension + 1, 1)
        )
        ax.set_yticks(
            np.arange(1, self.dimension + 1, 1),
    ...
    def plot_activities(self, ax=None, figsize=(15, 10)) -> None:
        ...
        for index, spikes in enumerate(spiketrains):
            ...
            if index % self.dimension == 0 and index > 0:
                ...
                ax.axhline(
                    current_y + self.space_between_cells / 2,
                    color = "k",
                    alpha = .5)
                current_y += self.space_between_cells

            # Only add labels in first cell
            label = index % self.dimension + 1 if index < self.dimension else None
            # Plot the acitvity
            ax.scatter(
                spikes,
                [current_y] * len(spikes),
                label = label,
                color = colors[index % self.dimension],
                s = 10
            )
        ...
        space_between_labels = self.dimension * self.space_between_numbers + self.space_between_cells

        ticks = np.arange(self.dimension**2) * space_between_labels + first_label
        numbers = np.arange(self.dimension) + 1
        labels = [f'[{row},{column}]' for row, column in product(numbers, numbers)]

        ax.set_yticks(ticks)
        ax.set_yticklabels(labels)

        ax.set_xlabel("Time [ms]")
        ax.set_ylabel("Coordinate [row, column]")

    def plot(self, grid = True, savefig_name = None, figsize=(15, 5)) -> None:
        """
        Plots the results
        if grid is True, makes one plot with grid and and activities, else two separate images
        :param grid=True: plots a grid
        :param figsize=(15,5): standard size of figure, suggested to have a ratio 3:1
