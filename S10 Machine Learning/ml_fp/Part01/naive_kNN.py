...
def classify_from_distances(
    distances: torch.Tensor, labels: torch.Tensor, k: int
) -> int | float:
    """Return the most common label of the nearest k training vectors given a 1d list of distances to all training vectors."""
    if not (k > 0 and int(k) == k):
        raise ValueError(f"k is not a positive integer! --> k = {k}")
...
def kNN_classify_batched(
    ...
    *,
    ...
    if not (0 < batch_size <= len(input_data)):
        raise ValueError(
            f"batch_size is not int between 0 and input_data length! --> {batch_size}"
        )
    ...
    classification_labels = torch.Tensor(
        list(chain.from_iterable(classification_iterable))
    )
