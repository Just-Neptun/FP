...
def stacking(stack, file, list_xsec_sumw, name, color):
    # Read in data
    ...
    if list_xsec_sumw:
        ...
        h.Scale(scalefactor(*list_xsec_sumw))
    styleHisto(h, color, "M_{ll} [GeV]", "Entries", filled=True)
    stack.Add(h)
...
stack = ROOT.THStack("stackname", "Comparison of Data and MC Calculations of Z Mass;M_{ll} [GeV / c^{2}];Entries")

# stacking(stack, file_d, None, names[0], ROOT.kBlack)

h_d = file_d.Get(histname)
styleHisto(h_d, ROOT.kBlack, "M_{ll} [GeV]", "Entries")
legend.AddEntry(h_d, names[0], "f")

stacking(stack, file_e, [xsec_e, sumw_e], names[1], ROOT.kBlue)
stacking(stack, file_m, [xsec_m, sumw_m], names[2], ROOT.kGreen)
stacking(stack, file_t, [xsec_t, sumw_t], names[3], ROOT.kRed)
