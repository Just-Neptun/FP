...
\begin{equation}
    S_{λ}(\vec{x}) = \hat{λ}.
\end{equation}
We want an estimator to be \define{unbiased}, \( \ev{S_{λ}} = λ \), \define{consistent}, \( \lim_{n \to \infty} \hat{λ} = λ \), and \define{efficient}, \( \ev{(S - λ)^2} = σ_{S_{λ}}^2 \leq σ_{S'_{λ}}^2 \), for all other estimators \( S'_{λ} \).
...
In \autoref{sec:analysis}, we introduce three general frameworks which let us estimate the parameters of models.
...
But first, we use our now-acquired knowledge to implement symbolic error propagation in Python.