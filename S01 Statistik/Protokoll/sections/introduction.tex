...
If this is the case, we work with the PMF or PDF of such a random variable given the value of this parameter, \( f_X(x \given λ) \), in the sense of conditional probability defined above.

A random variable is distributed according to the \define{Gaussian distribution}, if its PDF is of the form
\begin{gather}
    f(x \given μ, σ^2) = \frac{1}{\sqrt{2 π σ^2}} \exp(\frac{(x - μ)^2}{2 σ^2}).
\end{gather}
It is a family of distributions indexed by its mean and variance, \( \vec{λ} = (μ, σ^2)\transpose \).
...
If \( n \) random variables \( x_1, \dotsc, x_n \) all follow standard Gaussian distributions with \( \mu = 0 \) and \( \sigma = 1 \), the sum of their squares follows the \define{\( \chi^2 \)-distribution} for \( n \) so-called \quotedef{degrees of freedom}.
\begin{gather}
    z = \chi^2 = x_1^2 + \dotsb + x_n^2 \\
    f(z \given n) = \frac{z^{\frac{n}{2} - 1} \cdot e^{-\frac{z}{2}}}{2^{\frac{n}{2}} \cdot \Gamma{\big( \frac{n}{2} \big)}} \quad (z ≥ 0)
