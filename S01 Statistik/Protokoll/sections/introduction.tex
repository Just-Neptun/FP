...
Because measurements are influenced by the addition of many statistical errors (which behave as randomly distributed variables), it is usually assumed that a measurement comes from a Gaussian distribution centered around the true value (mean \( \mu \)) with an error (standard deviation \( \sigma \)).
