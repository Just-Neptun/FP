...
\end{gather}
This kind of polynomial function is linear in its parameters (the coefficients \( θ_i \)), meaning an exact solution exists.
% % Is this real? Explain in intro?
% \begin{gather}
%     f(x ; \vec{θ}) = \sum \limits _{k = 0}^{3} x^{k} \cdot θ_k
%     \label{eq:least_squares:general_form_fit_polynomial}
% \end{gather}

For a set of \( n \) inputs $\vec{x}$, our model predicts outputs: $ \vec{\mu} = \mat{A} \vec{\theta} $, with $ A_{ij} = x_i^j$. 
When given measurement data, $\vec{x}$ represents the independent variable and $\vec{y}$ represents the dependent variable.
