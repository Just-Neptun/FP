...
The probability that \( X \) takes on a given value \( x \in \reals \) is given by
\begin{equation}
    P(X = x) \coloneq P(\set{ω \in Ω \mid X(ω) = x}).
\end{equation}
We naturally define the \define{image} \( X(E) = \set{X(ω) \in \reals \mid ω \in E} \) of an event \( E \) and the \define{pre-image} \( X^{-1}(x) = \set{ω \in Ω \mid X(ω) = x} \) of a possible value of \( X \).
