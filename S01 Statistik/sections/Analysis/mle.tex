...
To understand the shapes of log-likelihood functions better, we generate some data from the distribution \autoref{eq:mle:decay_function}, using the true value of \( \tau = \num{2} \). The log-likelihood function is calculated and plotted for different values of the parameter \( \tau \) for datasets with \( n \in \{ 30, 300, \num{10000}\} \) data points (see \autoref{fig:mle:lllh_30_data_points}, \autoref{fig:mle:lllh_300_data_points} and \autoref{fig:mle:lllh_10000_data_points}).
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{Graphs/MLE/lllh_for_30_data_points.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{Graphs/MLE/lllh_for_300_data_points.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{Graphs/MLE/lllh_for_10000_data_points.png}
    \caption{Exact and Approximated Log-Likelihood Functions for \( n = \num{10000} \) Data Points}
...
We want to examine another generated dataset. For this one, the probability density function for the distribution of an angle $\alpha$ is given by \autoref{eq:mle:pdf_02}.
\begin{gather}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{Graphs/MLE/lllh_part_02.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{Graphs/MLE/data_and_mle_part_02.png}
...
Now we can use the standard \texttt{scipy.optimize.root\_scalar} function to find these roots and solve for the positive and negative deviations from \( \hat{a} \).
...
In order to analyze how the number of data points impacts the errors, we select some interesting numbers of events for which to try the MLE method (see attached code\typeout{appendix or nah?}). Clearly very small samples have very large errors and have no power to accurately determine the parameter $a$. The errors are not just large, but also unbalanced: The difference between $\sigma^+$ and $\sigma^-$ is greater than $5 \%$ for small samples.
