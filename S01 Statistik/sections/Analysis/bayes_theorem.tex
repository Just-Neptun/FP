...
If we make a set of observations \( \vec{x} \) where \( x_i = (N_i, k_i) \), we can calculate the likelihood \( L(\vec{x} | \falpha) \) of these observations given our model,
\begin{equation}
    \label{eq:bayes_theorem:likelihood_definition}
    L(\vec{x} | \falpha) = \prod_{x_i \in \vec{x}} P(x_i, \falpha).
\end{equation}
...
\begin{equation}
    ...
    L(x_1, \dots, x_n | \falpha) \propto \prod_{i = 1}^n \falpha^{k_i} (1 - \falpha)^{N_i - k_i} = \falpha^k (1 - \falpha)^{N - k},
\end{equation}
...
\begin{equation}
    ...
    P(\falpha | \vec{x}) = \frac{L(\vec{x} | \falpha) \cdot π(\falpha)}{\int L(\vec{x} | \falpha) \cdot π(\falpha) \difl{\falpha}}.
\end{equation}
...
If a single decay was measured going to α, the likelihood is simply \( L(\mathrm{α} | \falpha) = \falpha \).%
\footnote{Technically, this represents an abuse of our notation as defined above.
Correct would be writing \( L(N = 1, k = 1 | \falpha) \), or even \( L(\vec{x} | \falpha) \) where \( \vec{x} = (x_1) \), \( x_1 = (N_1, k_1) \), and \( N_1 = k_1 = 1 \).}
...
Therefore, \( \pi(\falpha) = 1 \forall \falpha \in [0, 1] \) and the posterior is
\begin{equation}
    P(\falpha | \mathrm{α}) = \frac{\falpha \cdot 1}{\int_0^1 \falpha \cdot 1 \difl{\falpha}} = 2 \falpha.
...
\begin{equation}
    P(\falpha | \vec{x}) \propto \falpha^k (1 - \falpha)^{N - k} \falpha^{α - 1} (1 - \falpha)^{β - 1} = \falpha^{α + k - 1} (1 - \falpha)^{β + N - k - 1},
\end{equation}
...
For a single measurement, our calculation will show that this method is equivalent to the analytic function \( P(\falpha|\mathrm{α}) = 2 \falpha \).
...
Now we are given four further measurements, \( \vec{x} = (\mathrm{β}, \mathrm{α}, \mathrm{α}, \mathrm{β}) \).
